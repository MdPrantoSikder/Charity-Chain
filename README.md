...
Alongside the application, this project contributes **CB-BFT (Cluster-Based Byzantine Fault Tolerant)** — a consensus mechanism implemented inside Besu that selects block proposers by measured validator performance rather than fixed rotation — and an **evaluation testbed** in which any Besu-supported consensus protocol can be compared under identical conditions.

> **CB-BFT implementation:** [`MdPrantoSikder/besu` → compare `main...cbbft`](https://github.com/MdPrantoSikder/besu/compare/main...cbbft)
> 9 files, 661 lines: the consensus module and the wiring that activates it.
...
| # | Contribution | Where |
|---|---|---|
| 1 | CB-BFT consensus implemented inside Besu (Java) | [Besu fork](https://github.com/MdPrantoSikder/besu/compare/main...cbbft) |
| 2 | Controlled testbed for comparing consensus protocols | [`wsl-setup/`](wsl-setup/) |
| 3 | Empirical evaluation: 4 protocols, 116 runs | [`cbbft-results/`](cbbft-results/) |
...
1. **Score** each validator on three CRITIC criteria derived from block headers — resource utilisation, latency, and proposal reputation.
2. **Cluster** validators adaptively by score.
3. **Select** proposers from the top 30%, excluding validators that are not performing.

The BFT state machine — three-phase commit, round changes, block validation — is inherited unchanged from QBFT. Only proposer selection differs, activated by a single genesis flag (`cbbftproposerselection`). All scoring uses 64-bit fixed-point arithmetic to guarantee bit-identical results across nodes.
...
Every run begins from a freshly generated network, so no chain state, peer memory, or database carries over.
...
Protocols implemented in other clients — Raft in GoQuorum, for example — were excluded deliberately: a different client brings a different transaction pool, EVM, and networking stack, so any measured difference would confound consensus with implementation.
...
**CB-BFT advantage over BFT baselines:**

| Failed validators | vs QBFT | vs IBFT2 |
|:---:|:---:|:---:|
| 0 | tie | tie |
| 1 | **+8.3%** | **+5.4%** |
| 2 | **+25.1%** | **+18.4%** |
...
A second experiment assigned validators unequal CPU (three tiers: 2.00 / 0.75 / 0.30 cores). The advantage over QBFT replicated: **7.2% / 16.0% / 30.7%** at one, two, and three faults — smaller in magnitude, identical in pattern, confirming the effect is not an artifact of uniform hardware.

### Independent benchmark

Benchmarked with **Hyperledger Caliper** on an ERC-20 transfer workload, 3 runs per protocol, 751/751 transactions successful in every run:

| Protocol | Avg latency | Throughput | Success |
|:---|:---:|:---:|:---:|
| QBFT | 1.48 s | 5.0 tx/s | 100% |
| IBFT2 | 1.33 s | 5.0 tx/s | 100% |
| **CB-BFT** | **1.22 s** | 5.0 tx/s | 100% |

The ordering matches the block-interval measurements, obtained with a standard tool on a different workload — independent corroboration of the instrumented results.
...
To compare a different protocol, swap the configuration file. Nothing else changes.

---

## Limitations

- **Client versions differ across arms.** QBFT and CB-BFT ran on the CB-BFT fork (26.8-develop); IBFT2 and Clique ran on upstream Besu 25.10.0, since the fork cannot parse their genesis extraData formats. The primary QBFT-versus-CB-BFT comparison is unaffected: same binary, one genesis flag.
